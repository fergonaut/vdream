...
// REGISTER_METACLASS(VWebProxy, VNet) // gilgil temp 2015.02.01
...
void VWebProxyKeepAliveThread::run()
{
  ...
  while (true)
  {
    ...
    for (VWebProxyConnections::iterator it = webProxy->connections.begin(); it != webProxy->connections.end(); it++)
    {
      ...
      if (connection->lastAccessTick + (VTick)webProxy->keepAliveTimeout < nowTick)
      {
...
{
  QUrl url =  QString::fromLatin1(request.requestLine.path);
  if (!url.isRelative())
  {
    host = url.host();
    port = url.port();
